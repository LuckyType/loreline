...
#: Where a password chosen in the browser is kept. Named with the same leading
#: underscore as the JWT secret, which is what marks a secret the app manages
#: for itself rather than a provider credential a GM typed in.
AUTH_PASSWORD_NAME = "_auth_password"
...
def ensure_auth_password(settings: Settings, store: SecretStore) -> None:
    """Load the stored password onto the live settings, unless one is set already.

    Same shape as ``ensure_jwt_secret`` above, and the same precedence the
    whole app uses: an environment variable beats anything the app wrote for
    itself. ``LORELINE_AUTH_PASSWORD`` therefore wins over whatever the first
    run stored, which is the documented way back into an instance whose
    password was mistyped at the claim - there is no other, because the
    password is only ever stored, never recoverable.

    Nothing is minted here. A missing password is a state, not a gap to fill:
    it is either an open dev box or an instance nobody has claimed, and
    ``loreline.web.setup`` is what tells those two apart.
    """
    if settings.auth_password:
        return
    stored = store.get(AUTH_PASSWORD_NAME)
    if stored:
        settings.auth_password = stored


def store_auth_password(settings: Settings, store: SecretStore, password: str) -> None:
    """Persist ``password`` and make it the live one, in that order.

    The write comes first so that a failure to persist leaves the instance
    unclaimed rather than claimed-until-restart, which would be a box the GM
    believes is theirs and is not.
    """
    store.set(AUTH_PASSWORD_NAME, password)
...
def set_session_cookie(response: Response, request: Request, settings: Settings) -> None:
    """Put a freshly issued session JWT on ``response`` as the auth cookie.

    Two routes sign a browser in - the login form and the first-run claim -
    and a cookie whose attributes differ between them is a session that works
    on one path and not the other. One function, so they cannot drift: the
    ``Secure`` flag in particular is decided by ``client_uses_https`` rather
    than set unconditionally, because marking it on the plain-HTTP LAN path
    this app primarily supports would stop the cookie being sent back at all.
    """
    response.set_cookie(
        COOKIE_NAME,
