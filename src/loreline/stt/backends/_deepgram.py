...
file (see ``deepgram_batch.py``). Same query parameters, same ``alternatives[]``
payload underneath. This module holds that overlap so the two connectors cannot
drift on the parts that are genuinely one thing, most of all the glossary
field, which differs per model rather than per transport. The addresses and
the ``Authorization: Token`` scheme are the two surfaces in capabilities.yaml.
...
def listen_params(
    *, model: str | None, language: str, terms: list[str], realtime: bool
    ...
    # model, which is a better thing to inherit than a value pinned here. This
    # connector used to pin nova-2, which stayed put long after nova-3 shipped
    # and quietly cost every session the newer model's keyterm biasing, since
    # nova-2 takes the legacy `keywords` field instead.
    if model:
