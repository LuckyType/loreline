...
def _config(model: str | None = "universal-2") -> ProviderConfig:
...
async def test_health_lists_one_transcript() -> None:
    ...
    async with client:
        assert await AssemblyAIBatchBackend(_config(), client=client).health()
    ...
    async with client:
        assert not await AssemblyAIBatchBackend(_config(), client=client).health()
